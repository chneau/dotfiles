...
# credits to https://github.com/cykerway/complete-alias
# code just minified

#_retval=0;_use_alias=0;_in(){ for e in "${@:2}";do [[ "$e" == "$1" ]]&&return 0;done;return 1;}

#_expand_alias(){ local beg="$1" end="$2" ignore="$3" n_used="$4";shift 4;local used=("${@:1:$n_used}");shift $n_used;if [[ "$beg" -eq "$end" ]];then _retval=0;elif [[ -n "$ignore" ]]&&[[ "$beg" -eq "$ignore" ]];then _expand_alias "$(($beg+1))" "$end" "$ignore" "${#used[@]}" "${used[@]}";_retval="$_retval";elif !(alias "${COMP_WORDS[$beg]}"&>/dev/null)||(_in "${COMP_WORDS[$beg]}" "${used[@]}");then _retval=0;else local cmd="${COMP_WORDS[$beg]}";local str0="$(alias "$cmd"|sed -E 's/[^=]*=//'|xargs)" { words0=();local sta=();local i=0 j=0;for((j=0;j<${#str0};j++));do if [[ $' \t\n' == *"${str0:j:1}"* ]];then if [[ ${#sta[@]} -eq 0 ]];then if [[ $i -lt $j ]];then words0+=("${str0:i:j-i}");fi;((i=j+1));fi;elif [[ "><=;|&:" == *"${str0:j:1}"* ]];then if [[ ${#sta[@]} -eq 0 ]];then if [[ $i -lt $j ]];then words0+=("${str0:i:j-i}");fi;words0+=("${str0:j:1}");((i=j+1));fi;elif [[ "\"')}" == *"${str0:j:1}"* ]];then if [[ ${#sta[@]} -ne 0 ]]&&[[ "${str0:j:1}" == ${sta[-1]} ]];then unset sta[-1];fi;elif [[ "\"'({" == *"${str0:j:1}"* ]];then if [[ "${str0:j:1}" == "\"" ]];then sta+=("\"");elif [[ "${str0:j:1}" == "'" ]];then sta+=("'");elif [[ "${str0:j:1}" == "(" ]];then sta+=(")");elif [[ "${str0:j:1}" == "{" ]];then sta+=("}");fi;fi;done;if [[ $i -lt $j ]];then words0+=("${str0:i:j-i}");fi;unset sta;};local i j=0;for((i=0;i<$beg;i++));do for((;j<= ${#COMP_LINE};j++));do [[ "${COMP_LINE:j}" == "${COMP_WORDS[i]}"* ]]&&break;done;((j+=${#COMP_WORDS[i]}));done;for((;j<= ${#COMP_LINE};j++));do [[ "${COMP_LINE:j}" == "${COMP_WORDS[i]}"* ]]&&break;done;COMP_LINE="${COMP_LINE[@]:0:j}""$str0""${COMP_LINE[@]:j+${#cmd}}";if [[ $COMP_POINT -lt $j ]];then :;elif [[ $COMP_POINT -lt $((j+${#cmd}))]];then((COMP_POINT=j+${#str0}));else((COMP_POINT+=${#str0}-${#cmd}));fi;COMP_WORDS=("${COMP_WORDS[@]:0:beg}" "${words0[@]}" "${COMP_WORDS[@]:beg+1}");if [[ $COMP_CWORD -lt $beg ]];then :;elif [[ $COMP_CWORD -lt $(($beg+1))]];then((COMP_CWORD=beg+${#words0[@]}));else((COMP_CWORD+=${#words0[@]}-1));fi;if [[ -n "$ignore" ]]&&[[ $ignore -gt $beg ]];then((ignore+=${#words0[@]}-1));fi;local used0=("${used[@]}" "$cmd");_expand_alias "$beg" "$(($beg+${#words0[@]}))" "$ignore" "${#used0[@]}" "${used0[@]}";local diff0="$_retval";if [[ -n "$str0" ]]&&[[ "${str0: -1}" == ' ' ]];then local used1=("${used[@]}");_expand_alias "$(($beg+${#words0[@]}+$diff0))" "$(($end+${#words0[@]}-1+$diff0))" "$ignore" "${#used1[@]}" "${used1[@]}";local diff1="$_retval";else local diff1=0;fi;_retval=$((${#words0[@]}-1+diff0+diff1));fi;}


#_set_default_completion(){ local cmd="$1";case "$cmd" in bind);complete -A binding "$cmd";;;help);complete -A helptopic "$cmd";;;set);complete -A setopt "$cmd";;;shopt);complete -A shopt "$cmd";;;bg);complete -A stopped -P '"%' -S '"' "$cmd";;;service);complete -F _service "$cmd";;;unalias);complete -a "$cmd";;;builtin);complete -b "$cmd";;;command|type|which);complete -c "$cmd";;;fg|jobs|disown);complete -j -P '"%' -S '"' "$cmd";;;groups|slay|w|sux);complete -u "$cmd";;;readonly|unset);complete -v "$cmd";;;traceroute|traceroute6|tracepath|tracepath6|fping|fping6|telnet|rsh|rlogin|ftp|dig|mtr|ssh-installkeys|showmount);complete -F _known_hosts "$cmd";;;aoss|command|do|else|eval|exec|ltrace|nice|nohup|padsp|then|time|tsocks|vsound|xargs);complete -F _command "$cmd";;;fakeroot|gksu|gksudo|kdesudo|really);complete -F _root_command "$cmd";;;a2ps|awk|base64|bash|bc|bison|cat|chroot|colordiff|cp|csplit|cut|date|df|diff|dir|du|enscript|env|expand|fmt|fold|gperf|grep|grub|head|irb|ld|ldd|less|ln|ls|m4|md5sum|mkdir|mkfifo|mknod|mv|netstat|nl|nm|objcopy|objdump|od|paste|pr|ptx|readelf|rm|rmdir|sed|seq|sha{,1,224,256,384,512}sum|shar|sort|split|strip|sum|tac|tail|tee|texindex|touch|tr|uname|unexpand|uniq|units|vdir|wc|who);complete -F _longopt "$cmd";;;*);_completion_loader "$cmd";;;esac;}

#_complete_alias(){ local cmd="${COMP_WORDS[0]}";if [[ $_use_alias -eq 0 ]];then local i j=0;for((i=0;i<$COMP_CWORD;i++));do for((;j<= ${#COMP_LINE};j++));do [[ "${COMP_LINE:j}" == "${COMP_WORDS[i]}"* ]]&&break;done;((j+=${#COMP_WORDS[i]}));done;for((;j<= ${#COMP_LINE};j++));do [[ "${COMP_LINE:j}" == "${COMP_WORDS[i]}"* ]]&&break;done;if [[ $j -le $COMP_POINT ]]&&[[ $COMP_POINT -le $(($j+${#COMP_WORDS[$COMP_CWORD]}))]];then local ignore="$COMP_CWORD";else local ignore="";fi;_expand_alias 0 "${#COMP_WORDS[@]}" "$ignore" 0;fi;((_use_alias++));_set_default_completion "$cmd";_command_offset 0;((_use_alias--));complete -F _complete_alias "$cmd";}
